...
func (bs *BaseService) Start() error {
	if atomic.CompareAndSwapUint32(&bs.started, 0, 1) {
		...
		bs.Logger.Info(fmt.Sprintf("Starting %v service", bs.name))
		err := bs.impl.OnStart()
		if err != nil {
			// revert flag
			atomic.StoreUint32(&bs.started, 0)
	...
	bs.Logger.Debug("service start",
		"msg",
		fmt.Sprintf("Not starting %v service -- already started", bs.name),
		"impl",
		bs.impl)
...
func (bs *BaseService) Stop() error {
	if atomic.CompareAndSwapUint32(&bs.stopped, 0, 1) {
		...
		bs.Logger.Info(fmt.Sprintf("Stopping %v service", bs.name))
		bs.impl.OnStop()
		close(bs.quit)
		return nil
	}
	bs.Logger.Debug("service stop",
		"msg",
		fmt.Sprintf("Stopping %v service (already stopped)", bs.name),
		"impl",
		bs.impl)
	return ErrAlreadyStopped
}
...
func (bs *BaseService) Reset() error {
	if !atomic.CompareAndSwapUint32(&bs.stopped, 1, 0) {
		bs.Logger.Debug("service reset",
			"msg",
			fmt.Sprintf("Can't reset %v service. Not stopped", bs.name),
			"impl",
			bs.impl)
